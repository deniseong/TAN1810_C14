...
pp_cal_1_join$population <- factor(pp_cal_1_join$population,levels = c("Syn", "Pico", "Nano")) 
#myColors <- ifelse(levels(pp_cal_1_join$population)=="Syn" , "seagreen4" , 
#              ifelse(levels(pp_cal_1_join$population)=="Pico", "salmon2",
#              "slategrey"))
cycle_boxplot <- pp_cal_1_join %>% ggplot(aes(y=pp, x= population)) +
                 geom_boxplot() + 
                 facet_grid(.~ cycle)+
                 scale_y_continuous(trans='log10') +
...
cycle_boxplot_pico_syn <- pp_cal_1_join %>% filter(population != "Nano") %>% ggplot(aes(y=pp, x= population)) +
                 geom_boxplot() + 
                 facet_grid(.~ cycle)+
                 #scale_y_continuous(trans='log10') +
                 ylab("Primary Production rate\n(fgC/h/cell)") +
                 xlab("Cycle")+
                 theme_bw()     
cycle_boxplot_nano <- pp_cal_1_join %>% filter(population == "Nano") %>% ggplot(aes(y=pp, x= population)) +
                 geom_boxplot() + 
                 facet_grid(.~ cycle)+
                 #scale_y_continuous(trans='log10') +
                 ylab("Primary Production rate\n(fgC/h/cell)") +
