...
# Denise edits on 18 Sep 2020

The following edits made to the data:
•	Exp 1: SUR: nano – vial C and D swap
•	Exp 3: SUR: pico: vial A – remove
•	Exp 4: SUR: pico, nano, syn – repeated vial B, take the average
•	Exp 6: SUR: nano – repeated vial A, remove lower value
•	Exp 7: SUR: nano – remove
•	Exp 7: DCM: nano – remove
•	Exp 8: DCM: nano – remove
•	Exp 8: SUR: nano: 2000 cell count – remove


The calculations of primary production based on single cell C14 uptake for cells sorted by flow cytometry. (https://vaulot.netlify.app/2018/05/20/compute-primary-production-based-on-single-cell-c14-uptake/)
...
Edits to be made:
•	Exp 4: SUR: pico, syn – repeated vial B, take the average

Data points to check for pico and syn calculations:
• Exp 2: DCM: pico vial A- only 1 value.
• Exp 6: SUR: pico, syn - dpm corrected values at cells sorted 2000 much higher than 4000 and 10000


## Read table for pico and syn. Data points to edit are made here. 
```{r}
C14_DPM <- readxl::read_excel("TAN1810_14C-Pico_25052020_QA.xlsx", sheet = "Complete data TAN1810", skip = 11) %>%
  dplyr::select (`Cycle#`, `EXP#`, SAMPLE, `Vial code             (i - initials, A to C - light incubation, D - dark)`, `sorting population`, `Cells sorted`, DPM1, DIC, `SA_dpm_mL`) %>%   
    ...
    filter(!(exp == 3 & sample == "SUR" & vial == "A" & population == "Pico"))  # Exp 3: SUR: pico: vial A – remove
    #Exp 4: SUR: pico, nano, syn – repeated vial B, take the average
#C14_DPM <- rowwise(cycle, exp, sample, population, vial, cells_sorted) %>% 
#  summarise(m = mean(dpm, DIC, SA))
...
•	exp 5: SUR. Vial D is high. Also marked in the exceel sheet. I removed.
• exp 2: DCM. cells sorted for vial A is 496. removed
...
Edits to be made:
•	Exp 1: SUR: nano – vial C and D swap
•	Exp 4: SUR: nano – repeated vial B, take the average
      
      
```{r}
# Changed the excel sheet read, with final dpm and DIC values
C14_DPM_nano <- readxl::read_excel("TAN1810_14C-Pico_25052020_QA.xlsx", sheet = "Complete data TAN1810", skip = 11)%>% 
  dplyr::select (`Cycle#`, `EXP#`, SAMPLE, `Vial code             (i - initials, A to C - light incubation, D - dark)`, `sorting population`, `Cells sorted`, DPM1, DIC, `SA_dpm_mL`) %>%   
     rename(cycle = `Cycle#`,
            exp = `EXP#`,
            sample = SAMPLE, 
    ...
    filter(!(exp == 2 & sample == "DCM" & vial == "A")) 
C14_DPM_nano
```
...
```{r} 
pp_cal_nano <- mutate(pp_cal_nano, pp = DIC*mean_dpm_corrected*(1/(SA*24*1000))*10^9*1.05) %>% #divide by 1000 cells sorted
  ungroup() %>%
  filter(pp >= 0) # remove negative pp values
...
# 3. Compute based on Method 2 - Compute lm for  ABC separately
