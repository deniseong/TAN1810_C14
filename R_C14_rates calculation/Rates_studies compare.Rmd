...
#Colours for cycle
```{r}
colours_water <- readxl::read_excel("init_files/colours_cycle.xlsx", sheet =  "Sheet3")
colours_water <- structure(colours_water$colour,.Names=colours_water$water)
```

```{r}

studies <- readxl::read_excel("data_used/C14_studies.xlsx", sheet = "Sheet1")  #updated one study
...
p <- studies %>% 
  ggplot(aes(fct_reorder(Study, Year), ymin = Lower, ymax = Upper, color = cat)) +
