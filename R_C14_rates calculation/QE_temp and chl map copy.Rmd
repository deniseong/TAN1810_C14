...
lat_sub <- which( nc_SST$dim$lat$vals > -47 & nc_SST $dim$lat$vals < -41.5)
...
plot_temp
#pdf("figures/GRC/map_temp.pdf", height=4, width=6) ; plot(plot_temp) ; dev.off()
...
```{r}
sampling_location <- readxl::read_excel("data_used/TAN1810_14C-Pico_25052020_QA_1.0.xlsx", sheet = "Complete data TAN1810", skip = 11) %>%
...
library(ggOceanMaps)
#THIS WORKS!
p <- basemap(limits = c(172, -179, -46, -41.5), rotate = TRUE,  lon.interval = 2, lat.interval = 1,grid.size = 0.5) + 
  geom_spatial_point(data = sampling_location, aes(x = long, y = lat))

p

pdf("map_c14.pdf", height=5, width=7) ; plot(p) ; dev.off()
```
