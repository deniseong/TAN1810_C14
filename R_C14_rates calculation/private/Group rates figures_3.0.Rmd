...
---
too many statistical tests, refer to 4.0
...
# Plots for paper
## Figure 4 - Boxplot raw data. Apparently not very good.
```{r}
...
pp_all <- ggplot() +
      ...
      scale_y_continuous(trans='log10') +
      annotation_logticks(sides = "l") +
  annotate("text", x = 3, y = 0.1, label = "ANOVA; R2=0.78,\n F[2,126]=221.84, p<0.0001")
pp_all
```
...
(Intercept)            0.9631   1  6.8834  0.009801 ** 
population            21.1433   2 75.5604 < 2.2e-16 ***
water_mass             4.2247   1 30.1956 2.148e-07 ***
population:water_mass  1.6548   2  5.9138  0.003530 ** 
Residuals             17.2089 123     

Predicted values of pp
...
In ST, Syn=pico<nano (non-parametric kruskal wallis)
In SA, syn<pico<nano (ANOVA)
```{r}
...
# checking diagnostic plots of lm. 
lm <- lm(log10(pp) ~ population, data=df)
plot(lm)
#load lmtest package
library(lmtest)
...
pp_pico <- ggplot() +
      ...
      scale_y_continuous(trans='log10') +
      annotation_logticks(sides = "l") +
  annotate("text", x = 1.7, y = 25, label = "t-test; R2=0.10,\n t41=2.43, p=0.02")
pp_pico
...
pp_nano<- ggplot() +
        ...
        labs(x = "Water mass",
             ...
             colour = "Water mass",
             shape = "Cycle",
             title = "Nanoeukaryotes") +
      scale_shape_manual(values=c(0,1, 15, 17, 19)) +
      scale_colour_manual(values = c("#C62828", "#1565C0")) +
      scale_y_continuous(trans='log10') +
      annotation_logticks(sides = "l") +
  annotate("text", x = 1.7, y = 300, label = "Welch t-test; R2=-0.01,\n t42=0.73, p=0.47")
...
p
pdf("pp_mean and CI.pdf", height=10, width=12) ; plot(p) ; dev.off()
...
npp_all <- ggplot() +
      ...
      scale_y_continuous(trans='log10') +
      annotation_logticks(sides = "l") +
  annotate("text", x = 3, y = 30, label = "ANOVA; R2=0.21,\n F{2, 126}=17.99, p<0.0001")

npp_all
```
#### interaction
Predicted values of npp
...
lm2 <- lm(log10(npp) ~ population+water_mass, data=sub) # chose this model
...
npp_mixed <- ggplot() +
      ...
      scale_y_continuous(trans='log10') +
      annotation_logticks(sides = "l") +
      annotate("text", x = 2.5, y = 20, label = "ANOVA; R2=0.28,\n Population F[4,125]=20.3, \n Water mass F[4,125] = 12.4, \n p<0.0001", size= 3)
npp_mixed
...
res <- t.test(log10(npp) ~ water_mass, data =  t_test_df, var.equal = FALSE)
print(res)
...
npp_syn <- ggplot() +
      ...
      scale_shape_manual(values=c(0,1, 15, 17, 19)) +
      scale_colour_manual(values = c("#C62828", "#1565C0"))+ 
      scale_y_continuous(trans='log10', limits = c(0.04,30)) +
      annotation_logticks(sides = "l") +
  annotate("text", x = 1.7, y = 20, label = "t-test; R2=0.10,\n t(40)=2.41, p=0.02")
npp_syn
...
npp_pico <- ggplot() +
      ...
      scale_y_continuous(trans='log10', limits = c(0.04,30)) +
      annotation_logticks(sides = "l")+
  annotate("text", x = 1.7, y = 20, label = "t-test; R2=0.18,\n t(41)=3.17, p=0.003")
npp_pico
...
npp_nano<- ggplot() +
      ...
      scale_y_continuous(trans='log10', limits = c(0.04,30)) +
      annotation_logticks(sides = "l")+
  annotate("text", x = 1.7, y = 20, label = "t-test; R2=-0.02,\n t(42)=0.53, p=0.60")
...
p <- ((npp_all+ plot_spacer())/(npp_syn|npp_pico|npp_nano)) + plot_annotation(tag_levels = 'A') +  plot_layout(guides = 'collect')  &
  theme(legend.position='right')  & theme(axis.text = element_text(size = 12), legend.text = element_text(size = 12), legend.title = element_text(size = 12), axis.title = element_text(size = 12))
p
pdf("npp_mean and CI.pdf", height=10, width=12) ; plot(p) ; dev.off()
