...
colours_cycle <- readxl::read_excel("0_init_files/colours_cycle.xlsx")
...
```{r}
size_raw <-  readxl::read_excel("Output/Raw data/fsc_cell size.xlsx") %>%
...
rates <- readxl::read_excel("Output/Raw data/Rates_cell_npp_method2.xlsx") %>%
  select(-MD) %>%
...
join_rates <- join_rates %>%
  filter(pop_fsc!="Syn_SV") %>%
  mutate(biomass_new = case_when(population == "Pico" ~ 0.157*vol,
                                 population == "Nano" ~ 0.157*vol,
                                 population == "Syn" ~ vol*265/1000)) %>%
  mutate(biomass_fg = 1000*biomass_new) %>%
  mutate(growth_rate = (pp*24)/(biomass_fg)) %>% # multiply by 24 to change from per hour to per day
  mutate(division = 1/growth_rate)
