...
# change to calculate based on method 2!!!!
...
pp_rates <- readxl::read_xlsx("data_used/C14_rates per group_method1.xlsx")
...
#png("../figures/C14_npp per group.png", width = 1000, height = 500)
...
cell_conc <- fcm_corrected %>%
  ...
  geom_boxplot(aes(y=conc_corrected, x= population , fill = population), show.legend = FALSE) +
                 ...
                 facet_grid(.~cycle_name)+
  scale_fill_manual(values =  c("#E69F00","#009E73","#D55E00")) +
  ylab(expression(Number~of~cells~ml^-1)) +
  xlab("Cycle")+
    theme(axis.title=element_text(size=20),
        axis.text = element_text(size = 20),
        strip.text = element_text(size = 24))

#png("../figures/cell conc.png", width = 1000, height = 500)
cell_conc
```
