...
These figures are based on method 1. Move to archive.
...
```{r}
pp_cal_1 <- mutate(C14_DPM_model_output_1, pp = DIC*(slope/(SA*24))*10^9*1.05) %>%
  ungroup() %>%
...
C14_DPM_nano <- readxl::read_excel("../data_used/TAN1810_14C-Pico_25052020_QA_1.0.xlsx", sheet = "Complete data TAN1810", skip = 11)%>% 
  dplyr::select (`Cycle#`, `EXP#`, SAMPLE, `Vial code             (i - initials, A to C - light incubation, D - dark)`, `sorting population`, `Cells sorted`, DPM1, DIC, `SA_dpm_mL`) %>%   
     rename(cycle = `Cycle#`,
            exp = `EXP#`,
            sample = SAMPLE, 
...
```{r}
pp_cal_1_join_raw<-full_join(pp_cal_1,pp_cal_nano) %>%
  select(-SA,-intercept, -slope, -DIC, -cells_sorted, -mean_dpm_corrected)%>%
  arrange(cycle, population)%>%
...
#Summary
```{r}
pp_summary <- summarySE(pp_cal_1_join, measurevar="pp", groupvars=c("cycle_name", "population"), na.rm=TRUE) 
pp_summary
```
...
Subantarctic waters - Cycle 1, 2 and 5
```{r}
#edit pp cal to exclude cycle 3:exp 8: SUR : pico and syn from boxplot. Values are too far from the median.
pp_cal_1_join_edit <- pp_cal_1_join %>%
  filter(!(exp == 8 & sample == "SUR" & population == "Pico")) %>%
  filter(!(exp == 8 & sample == "SUR" & population == "Syn")) %>%
  dplyr::rename(pp_filtered = pp)

pp_join <- left_join(pp_cal_1_join, pp_cal_1_join_edit)
pp_join$cycle_name<-as.character(pp_join$cycle_name)
pp_join$cycle_name <- factor(pp_join$cycle_name, levels = c("ST1", "ST2", "SA1", "SA2", "SA3"))

pp_join$population <- factor(pp_join$population,levels = c("Syn", "Pico", "Nano")) 

library(wesanderson)
cycle_boxplot <- pp_join %>%
                 ggplot() +
                 geom_boxplot(aes(y=pp_filtered, x= population , fill = population), show.legend = FALSE) + 
                 geom_point(aes(y=pp, x= population))+
...
png("../figures/C14_hot_QE.png", width = 1000, height = 500)
plot(cycle_boxplot)
```


```{r}
cycle_boxplot_pico_syn <- pp_join %>%
...
cycle_boxplot_pico_syn
cycle_boxplot_nano
```

```{r}
c14_figure <- ggarrange(
  cycle_boxplot, cycle_boxplot_pico_syn, cycle_boxplot_nano, 
  labels = c("A", "B", "C"), 
  ncol = 1, nrow = 3,
  align = "v") 

ggsave("../figures/c14_figure_1.png")
...
cycle_table <- pp_cal_1_join %>% 
  ...
  pivot_wider(names_from = cycle_name, values_from = pp_mean)
cycle_table
```

```{r}
# edit pp cal to exclude cycle 3:exp 8: SUR : pico and syn from boxplot. Values are too far from the median.
cycle_table_1 <- pp_cal_1_join_edit %>% 
  group_by(cycle_name, population) %>% 
  summarise(pp_mean = mean(pp_filtered, na.rm = TRUE)) %>%
  pivot_wider(names_from = cycle_name, values_from = pp_mean)
cycle_table_1
write.xlsx(cycle_table_1)
```
Pico and Syn values are too low.
...
# Analysis based on Surface vs DCM - no difference
```{r}

depth_boxplot <- ggplot(pp_cal_1_join, aes(x = population, y=pp, fill=sample)) +
                 geom_boxplot() + 
                 scale_y_continuous(trans='log10') +
                 ylab("Primary Production (fgC/h/cell)") 
...
#Cycle and depth - no difference
```{r}
cycle_depth_boxplot <- pp_join %>%
                 ggplot() +
                 geom_boxplot(aes(y=pp_filtered, x= population, fill = sample)) + 
                 geom_point(aes(y=pp, x= population, fill = sample))+
                 facet_grid(.~cycle_name)+
                 scale_y_continuous(trans='log10') +
                 ylab("Primary Production rate\n(fgC/h/cell)") +
                 xlab("Cycle") +
...
cycle_depth_boxplot_pico_syn <- pp_join %>% filter(population != "Nano") %>%
                 ggplot() +
                 geom_boxplot(aes(y=pp_filtered, x= population, fill=sample)) + 
                 geom_point(aes(y=pp, x= population, fill = sample))+
                 facet_grid(.~cycle_name) +
...
cycle_depth_boxplot_nano <- pp_join %>% filter(population == "Nano") %>%
                 ggplot() +
                 geom_boxplot(aes(y=pp, x= population, fill = sample)) + 
                 geom_point(aes(y=pp, x= population, fill = sample))+
                 facet_grid(.~ cycle_name)+
...
cycle_depth_boxplot
cycle_depth_boxplot_pico_syn
cycle_depth_boxplot_nano
```
