...
CTD <- CTD %>% mutate(cycle_name = recode(cycle, "1" = "SA1",
  ...
  filter(cycle_name_2 != "SA1-B") %>% #  SA1 separated into two sub cycles. C14 exp only includes SA1-A. remve SA1-B
  filter(!is.na(cycle)) %>% #remove rows without cycle number
  mutate(cycle_name = fct_relevel(cycle_name, "ST1", "ST2", "SA1", "SA2", "SA3")) %>%
  filter(julian_day %in% c("297", "298", "305", "306", "311", "312", "315", "316", "319")) # only days corresponding to exp, except 5th and 6th nov, without afternoon CTD.
...
par_plot <- CTD %>%
   filter(julian_day %in% c("297", "298", "305", "306", "311", "312", "315", "316", "319")) %>% # selecting only for exp dates
  group_by(cycle_name, CTD) %>%
  group_modify(~ add_row(.x,.before=0)) %>% # add gaps in the dataframe for geom_path linebreaks
  # filter(p <= 100) %>%
...
nutrients <- nutrients %>%
  ...
  mutate(cycle_name = fct_relevel(cycle_name, "ST1", "ST2", "SA1", "SA2", "SA3")) %>%
  filter(time %in% (0:3) ) # only morning CTD


check <- nutrients  %>% 
  select(CTD, date, time, cycle_name_2) %>%
  distinct() %>%
  arrange(date)
check
...
#Summary table - ignore. Nutrients are averaged over the whole depth. Maybe should do until DCM?
```{r}
summary <-   nutrients %>%
...
library(xtable)
summary <- readxl::read_excel("summary_ctdnut_3.0.xlsx")
...
summary <-   nutrients_sub %>%
  ...
  pivot_longer(cols = c("DRP_uM", "NH4_uM", "NO3_uM", "DRSi_uM", "nitrate_ammonia", "si_nitrate"), names_to = "nutrients", values_to = "conc") %>%
  group_by(cycle_name, nutrients) %>%
  summarise(conc_mean = round(mean(conc), 3),
            conc_sd= round(sd(conc),3)) %>%
  unite("mean_sd", conc_mean:conc_sd, sep = " ± ") %>%
  pivot_wider(names_from = nutrients,  values_from = mean_sd) %>%
  ungroup() %>%
  select(cycle_name,  "NH4_uM", "NO3_uM","DRP_uM","DRSi_uM", "nitrate_ammonia", "si_nitrate")
...
summary_all
writexl::write_xlsx(summary_all, "summary_ctdnut_3.0.xlsx")
```
...
plot <- (temp_plot | s_plot |plot_spacer() | plot_spacer()) / (ammonia_plot | nitrate_plot | p_plot | si_plot) +  plot_annotation(tag_levels = 'A') & theme(legend.position="none")
plot
pdf("figures/4_ctd nutrients/ctd nut all_v2.pdf", height=7, width=9) ; plot(plot) ; dev.off()

```

##combine phosphorus and nitrate -ignore
```{r}
scale=10
p <- nutrients %>%
  ggplot(aes(y=depth)) + 
...
ST1 <-nutrients %>%
  filter(CTD %in% c("9141", "9144"))

nitrate_plot <- ST1 %>%
    arrange(cycle_name, CTD, depth)%>%
  group_by(cycle_name, CTD) %>%
  group_modify(~ add_row(.x,.before=0)) %>% # add gaps in the dataframe for geom_path linebreaks
  ggplot(aes(x=NO3_uM, y=Depth_nominal, colour=CTD)) + 
  geom_point(size=0.2, alpha = 0.6) +
  # geom_path(size=0.2)+
  geom_smooth(orientation = "y",
             se = FALSE
...
ammonia_plot <- ST1 %>%
  ...
  group_by(cycle_name, CTD) %>%
  group_modify(~ add_row(.x,.before=0)) %>% # add gaps in the dataframe for geom_path linebreaks
  ggplot(aes(x=NH4_uM, y=Depth_nominal, colour=CTD)) + 
  geom_point(size=0.1, alpha = 0.6) +
  # geom_path(size=0.2)+
  geom_smooth(orientation = "y",
             se = FALSE
...
p_plot <- ST1 %>%
  ...
  group_by(cycle_name, CTD) %>%
  group_modify(~ add_row(.x,.before=0)) %>% # add gaps in the dataframe for geom_path linebreaks
  ggplot(aes(x=DRP_uM, y=Depth_nominal, colour=CTD)) + 
  geom_point(size=0.1, alpha = 0.6) +
  # geom_path(size=0.2)+
  geom_smooth(orientation = "y",
              se = FALSE
...
si_plot <-ST1 %>%
  # group_by(cycle_name, CTD) %>%
  # group_modify(~ add_row(.x,.before=0)) %>% # add gaps in the dataframe for geom_path linebreaks
  ggplot(aes(x=DRSi_uM, y=Depth_nominal, colour=CTD)) + 
  geom_point(size=0.1, alpha = 0.6) +
  # geom_path(size=0.2) +
  geom_smooth(orientation = "y",
              se = FALSE,
