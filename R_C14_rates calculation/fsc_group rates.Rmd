...
join_rates <- join_rates %>%
 ...
 # mutate(pp_vol_check = pp_vol_norm*10^(6)*1/24) # I think this is wrong.
...
library(multcomp)
tmp <- as.data.frame(confint(glht(lm))$confint)
...
print(summary)
 p <- t_test_df %>%
   ggplot(aes(x=depth_cat, y=growth_rate)) + geom_point() +geom_boxplot()+ scale_y_continuous(trans='log10') + annotation_logticks(sides = "l")  
 print(p)
...
print(summary)
 p <- t_test_df %>%
   ggplot(aes(x=water_mass, y=growth_rate)) + geom_point() +geom_boxplot()+ scale_y_continuous(trans='log10') + annotation_logticks(sides = "l")  
 print(p)
