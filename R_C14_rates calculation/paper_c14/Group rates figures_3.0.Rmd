...
not normally distributed, using kruskal test. Significantly different between all groups.
```{r}
hist(pp_rates$pp)
qqnorm(pp_rates$pp)
...
shapiro.test(log10(pp_rates$pp)) # still not significant. Cannot use anova
...
dunnTest(pp~population, data = pp_rates,
         method="bonferroni")
```
In each water mass compare between groups. Same pattern
```{r}
...
print(summary)
 p <- t_test_df %>%
   ggplot(aes(x=water_mass, y=npp)) + geom_point() +geom_boxplot()+ scale_y_continuous(trans='log10') + annotation_logticks(sides = "l")  
 print(p)
