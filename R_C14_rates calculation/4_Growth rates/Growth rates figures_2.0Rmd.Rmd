...
## Extra plots - maybe supplementary
by water mass
```{r}
p <- join_rates %>%
...
Although for Syn surface is different from DCM, based on the raw data it is driven by low values from exp 2 and 5 where there are no DCM measurements available.
```{r}
# by depth
for (i in c("Syn", "Pico", "Nano")) {
  p <- join_rates %>%
...
for (i in c("Syn", "Pico", "Nano")) {
  ...
  ggplot() +
    geom_point(aes(x=depth_cat, y=growth_rate))+
    geom_boxplot(aes(x=depth_cat, y=growth_rate))+
    # facet_grid(.~exp, scales = "free_x", space = "free") +
    scale_y_continuous(trans='log10') +
    annotation_logticks(sides = "l")+
    ylab("Growth rate (day-1)") +
