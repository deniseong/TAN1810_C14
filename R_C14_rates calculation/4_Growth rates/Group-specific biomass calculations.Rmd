...
md_meta <- readxl::read_excel(here("0_data_used", "MD_PhotoAdapt_Biomass_Rates_17MAR20.xlsx"), sheet = "Summary of PhotoAdaption ", skip = 3) %>% 
...
md_comb<- data.frame()
for (i in c(2:7, 9:18)) {
md <- readxl::read_excel(here("0_data_used", "MD_PhotoAdapt_Biomass_Rates_17MAR20.xlsx"), sheet = (str_c("MD", i)), skip = 2, n_max = 8) %>%
  select('Depth','SYN MEAN FS':'NANO MEAN FS') %>%
  mutate(MD = i) %>%
  rename(depth_m = 'Depth')
 md_comb <- bind_rows(md_comb, md)
}
md1 <- readxl::read_excel(here("0_data_used", "MD_PhotoAdapt_Biomass_Rates_17MAR20.xlsx"), sheet = "MD1", skip = 2, n_max = 8) %>%
