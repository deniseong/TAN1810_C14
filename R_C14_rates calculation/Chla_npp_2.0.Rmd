...
# %>%
  # arrange(factor(sample, levels = c('SUR', 'DCM')))
summary
```
...
chl_summary <- chl %>%
  ...
  distinct() %>%
  mutate(water_mass = recode(cycle_name, "SA1" = "SA",
                             "SA2" = "SA",
                             "SA3" = "SA",
                             "ST1" = "ST",
                             "ST2" = "ST"
...
sf_summary <- sf_chl %>%
  filter(size == "20") %>%
  select(sf_int, cycle_name, size) %>%
  distinct() %>%
  filter(cycle_name == "SA3") %>%
...
plot
pdf("figures/2_NPP chla/20221129_int chla npp combine.pdf", height=7, width=5) ; plot(plot) ; dev.off()
