...
reg_plot
pdf("figures/C14_regression.pdf", height=15, width=10) ; plot(reg_plot) ; dev.off()
```
...
writexl::write_xlsx(pp_cal_1_join,"C14_rates per group.xlsx")
```
...
```{r fig.height=15, fig.width=10}
 reg_plot_2<- ggplot(data = C14_DPM_corrected, aes(x=cells_sorted, y=dpm_corrected, color=population, shape=vial)) +
  ...
  facet_grid(rows=vars(exp), cols=vars(sample), scales = "free_y") +
  scale_color_viridis_d()
pdf("figures/C14_regression plot 2.pdf", height=15, width=10) ; plot(reg_plot_2) ; dev.off()
...
```{r}
# merge tables
pp_cal_nano_2 <-  C14_DPM_corrected_nano %>%
  group_by(SA, population, cycle, DIC, exp, sample, vial) %>%
  mutate(mean_dpm_corrected=mean(dpm_corrected)) %>% #calculate mean
  select(-dpm, -dpm_dark, -dpm_corrected) %>%
  distinct() #remove duplicates
...
```{r} 
pp_cal_nano_2 <- mutate(pp_cal_nano_2, pp = DIC*mean_dpm_corrected*(1/(SA*24*cells_sorted))*10^9*1.05) %>% 
  ungroup() %>%
  filter(pp > 0) # remove negative pp values
...
## Join pico, syn and nano values together
