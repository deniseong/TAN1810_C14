...
# Denise: update on 20221019
For exp 8: SUR: vial B: nano, I could calculate the pp value based on LM (1000, 2000 cells sorted) and only 1000 cells sorted. 
Nano:8: sur: vial B: pp = 22.396 - from LM 
Nano:8: sur: vial B: pp = 20.851 - from 1000 cells
 values are comparable. I will use only the 1000 cells value to keep consistent with the other nano values.

# Denise: note on 12 Sept 2022
While preparing for the GRC, I have decided to use method 2 from now and treat vials A,B,C as replicates instead of combining.
...
•	Add Exp 8: SUR: nano – for vial B and D, there is cells sorted for both 1000 and 2000. Move to syn and pico calculations for lm value. <- DID NOT DO THIS. CHECK. done
...
## Method 1 - grouping ABC together - not using this method
### Plots
...
reg_plot
pdf("figures/C14_regression_method 1.pdf", height=15, width=10) ; plot(reg_plot) ; dev.off()
```
...
## Method 1 - group ABC vials together, calculate mean (not using)
### calculate mean
```{r, eval=FALSE}
# merge tables
...
writexl::write_xlsx(pp_cal_1_join,"C14_rates per group_method 1.xlsx")
```
...
writexl::write_xlsx(pp_cal_2_join,"C14_rates per group_method2.xlsx")
```
