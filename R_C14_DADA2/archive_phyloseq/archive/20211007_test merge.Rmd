...
ps <- read_rds("output_petb_2x300/output_petb_Denise_testing/testing_v2/petB_merge_testphyloseq_asv_set_sortedsyn.RDS") 
  ...
  ggplot(aes(x=file_code, y=n_reads, color=Subclade, fill=Subclade)) + 
    ...
    #    facet_grid(.~water_mass, scales = "free_x") +
     #   facet_grid(.~cycle_location_name, scales = "free_x") +
        ggtitle("C14 sorted syn Denise primer- merge test") +
        scale_color_manual(values=colours_petb_subclade, limits = force) + 
        scale_fill_manual(values=colours_petb_subclade, limits = force)
p

pdf("phyloseq/figures_20211007/C14 syn Denise merge test.pdf", height=5, width=8) ; plot(p) ; dev.off()
```


```{r}
# Load the phyloseq files
ps_ctd_nested <- read_rds("output_petb_2x300/output_petb_Denise_testing/testing_v2/petB_merge_testphyloseq_asv_set_ctdnested.RDS") 
...
p <- long_ctd_nested %>%
  ...
  ggplot(aes(x=file_code, y=n_reads, color=Subclade, fill=Subclade)) + 
        ...
        theme(text=element_text(size=10),axis.text.x = element_text(angle = 45, hjust = 1)) + 
       #facet_grid(.~cycle_location_name, scales = "free_x") +
       # ggtitle("CTD petB Denise primer R1 - subclade") +
        scale_color_manual(values=colours_petb_subclade, limits = force) + 
        scale_fill_manual(values=colours_petb_subclade, limits = force)
p

pdf("phyloseq/figures_20211007/CTD Denise merge test.pdf", height=5, width=8) ; plot(p) ; dev.off()
```
