...
---
Supplementary figure and initial checks for filtered samples.
# Initialize
...
pdf("phyloseq/paper_c14/filt_pico_class.pdf", height=6, width=8) ; plot(p2) ; dev.off()
```
...
# pdf("phyloseq/filt_pico_class.pdf", height=6, width=8) ; plot(p2) ; dev.off()
...
patchwork <- p /p1/p2+ plot_annotation(tag_levels = 'A') &theme(legend.text=element_text(size=9)) &theme(plot.margin = margin(0,0,0,0, "cm"))
pdf("phyloseq/supp_filt_compare_all.pdf", height=11.5, width=8) ; plot(patchwork) ; dev.off()
