...
file_param <- str_c("process_DADA2_DV/param_dada2_datasetD1.R")
...
# I think the directory code from Daniel is calling from the server. So I write a new code below to get directly from the directory.

fastq_dir <- "fastq_petB_nested"  # fastq directory

dir_fastqN <-  "output_petB_nested/fastqN/"  # for filtered files removing reads with N
dir_cutadapt <- "output_petB_nested/cutadapt/" # files after cutadapt

filtered_dir <- "output_petB_nested/filtered/"  # fastq filtered
qual_dir <- "output_petB_nested/qual_pdf/"  # qual pdf
petb_dir <- "output_petB_nested/petb/"  # petb results
blast_dir <- "output_petB_nested/blast/"  # blast2 results
database_dir <- "databases/Farrant_database_noESTU.fasta"  # databases
...
if (do_summary){
  ...
  writexl::write_xlsx(summary, path = "summary_raw_files.xlsx")
}
