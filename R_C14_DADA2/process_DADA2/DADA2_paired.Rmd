...
## Create ASV table
```{r}
#taxa.df<-readxl::read_xlsx("output_18SV4/TAN1810_C14_18SV4_taxo.xlsx")
#ASV.df<-readxl::read_xlsx("output_18SV4/TAN1810_C14_18SV4_ASV.xlsx")
#taxa_asv<- full_join(taxa.df, ASV.df)
seqtab.nochim_trans <- as.data.frame(t(seqtab.nochim)) %>% 
      rownames_to_column(var = "sequence") %>%
      rowid_to_column(var = "asv_number") %>%
      mutate(asv_code = sprintf("asv_%03s_%05d", dataset_id, asv_number)) %>% 
...
## Write fasta file with taxonomy only for subclade IVb
just for checking variation in one subclade. Not using.
```{r eval=FALSE}
seqtab.nochim_trans_IVa <- seqtab.nochim_trans %>%
  filter(Subclade =="IVa")
seq_out_IVa <- Biostrings::DNAStringSet(seqtab.nochim_trans_IVa$sequence)
names(seq_out_IVa) <- str_c(seqtab.nochim_trans_IVa$asv_code,seqtab.nochim_trans_IVa$Subclade, sep="|")
Biostrings::writeXStringSet(seq_out_IVa, output_path("_taxo_IVa.fasta"), 
                            compress=FALSE, width = 20000)
```
