---
title: "Script DADA2 for unpaired reads adapted from DV"
author: "Denise Ong"
date: "9/2/2021"
output: html_document
---

This script is adapted from "script_dada2.R" by Daniel. I copied over the sections to process unpaired reads and annotated some parts. 
...
file_param <- str_c("process_DADA2/datasetD2/param_dada2_datasetD2.R") # Change this to the parameter source code
...
## Define directories
```{r} 
source("process_DADA2/datasetD2/directories_DADA2_datasetD2_R1.R")
```
...
for(i in seq_along(fns)) {
        system2(cutadapt, args = c(flags_FWD, 
                                   ...
                                   "--minimum-length 200"))     # DO: added this line to enforce minimum length because of error later for summary and plot quality, probably because some reads with 0 bp (https://github.com/benjjneb/dada2/issues/159)
}
...
t_seqtab.nochim <- t(seqtab.nochim)
...
track <- cbind(out,
      ...
      colnames(track) <- c("cutadapt", "filtered", "denoised", "tabled", "nonchim")
...
# Assign taxonomy
NOTE: for function assignTaxonomy, change option tryRC depending on R1 or R2 reads. tryRC = TRUE will invert the read before assigning for reverse R2 reads.
```{r}
seqtab.nochim <- readRDS("output_petb_2x300/output_petb_F-R/R2/TAN1810_petB_F-R_seqtab.nochim.rds")
...
taxa_list <- list()
    ...
    for (i in 1:(length(slices)-1)){
      ...
      taxa_one <- assignTaxonomy(seqs=seq_one,
                                 ...
                                 tryRC = TRUE #only for R2 reads CHANGE ACCORDINGLY
                                 )
...
saveRDS(ps, output_path("_phyloseq_asv_set_CTD_petBF-R_R2.RDS"))
