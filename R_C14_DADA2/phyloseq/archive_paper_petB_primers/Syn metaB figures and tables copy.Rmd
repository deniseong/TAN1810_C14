...
source(here("phyloseq", "init_files", "define_functions.R")) #functions to 1.transform to dataframe, 2.normalise
...
#writexl::write_xlsx(stats_join, "subclade_summary_filt_2.0.xlsx")
```
...
# for log scale
nseqs_per_sample <- join_filt_pct_long %>%
  unite(col = "file_code", c("type", "sample_number")) %>%
    group_by(file_code) %>%
...
p2 <- join_filt_pct_long %>%
    filter(!(Subclade %in% c("Ia", "Ib", "IVa", "IVb"))) %>%
  mutate(percent = if_else(percent == 0,
                               2/3 *lod,
                               percent)) %>%
  #filter(percent !=0) %>%
  ggplot(aes(x=percent, y = fct_relevel(Subclade, "IIb", "VIc", "V", "II-WPC2", "VIb", "Ic", "UC-A", "IIe", "IIh", "WPC1", "EnvB", "EnvA", "CRD1"), colour=type)) +
  geom_boxplot(position=position_dodge(0.7), width=0.6) +
  geom_point(position=position_jitterdodge(0.15), alpha = 3/10)+
  scale_colour_paletteer_d("ggthemes::Color_Blind")+
...
p3 <- asv_count_long %>%
  ggplot(aes(x=ASV, y = fct_relevel(Subclade, "IIb", "VIc", "V", "II-WPC2", "VIb", "Ic", "UC-A", "IIe", "IIh", "WPC1", "EnvB", "EnvA", "CRD1", "Ia", "IVa", "IVb", "Ib"), group= Subclade)) +
...
#pdf("pct_reads_asv_2.0.pdf", height=10, width=7) ; plot(g) ; dev.off()
...
pct_sim <- readxl::read_excel(here("phyloseq", "paper_petB_primers", "figures_petb compare_2.0", "subclade_percent similarity_2.0.xlsx"), sheet = "for_R")
...
p1 <- long_pct_sim %>%
  ggplot(aes(x=pct, y = fct_relevel(Subclade, "IIb", "VIc", "V", "II-WPC2", "VIb", "Ic", "UC-A", "IIe", "IIh", "WPC1", "EnvB", "EnvA", "CRD1", "Ia", "IVa", "IVb", "Ib"), group= Subclade)) +
  geom_point(aes(color = type), size = 3) + 
...
p2 <- pct_sim %>%
  ggplot(aes(x=ref_seq, y = fct_relevel(Subclade,"IIb", "VIc", "V", "II-WPC2", "VIb", "Ic", "UC-A", "IIe", "IIh", "WPC1", "EnvB", "EnvA", "CRD1", "Ia", "IVa", "IVb", "Ib"))) +
...
pdf("pct_similarity_nucleotide_2.0.pdf", height=5, width=7) ; plot(p) ; dev.off()
```
...
## Filt and sorted- mean proportion of reads per lineage per cycle
```{r}
...
com <-  average_cycle_join[,3:ncol(average_cycle_join)]
...
data.scores$cycle_name = average_cycle_join$cycle_name
data.scores$type = average_cycle_join$type
 
p <- data.scores %>%
    ggplot(aes(x=NMDS1, y= NMDS2, colour = type, shape =cycle_name)) +
    geom_point()
p
```

```{r}
check <- average_cycle_join %>%
  mutate(name = paste(cycle_name, type, sep = "_")) %>%
  select(-cycle_name, -type)

check <- check %>%
  column_to_rownames(var = "name")

dist <- vegan::vegdist(check, "bray")
...
```{r}
filt_clust <- join_filt_pct %>%
  select(sample_number, Subclade:cycle_name)  %>%
  pivot_longer(percent_ong:percent_mazard, names_to = "type", values_to= "percent") %>%
  pivot_wider(names_from = Subclade, values_from = percent) %>%
  mutate(type = recode(type, "percent_ong" = "Ong", "percent_mazard"= "Mazard")) %>%
  mutate(name = paste(cycle_name, sample_number, type, sep = "_")) %>%
  column_to_rownames(var = "name") %>%
  select(!(cycle_name:type)) %>%
  select(-sample_number) %>%
  replace(is.na(.), 0)

dist_filt <- vegan::vegdist(filt_clust, "bray")
hc_filt <- hclust(dist_filt, method = "complete")
plot(hc_filt, hang = -1)

#nmds
library(vegan)
set.seed(1)
nmds = vegan::metaMDS(filt_clust, distance = "bray")
nmds$stress


filt_clust2 <- join_filt_pct %>%
  select(sample_number, Subclade:cycle_name)  %>%
  pivot_longer(percent_ong:percent_mazard, names_to = "type", values_to= "percent") %>%
  pivot_wider(names_from = Subclade, values_from = percent) %>%
  mutate(type = recode(type, "percent_ong" = "Ong", "percent_mazard"= "Mazard"))
...
data.scores = as.data.frame(scores(nmds))
#add columns to data frame 
data.scores$cycle_name = filt_clust2$cycle_name
data.scores$type = filt_clust2$type
