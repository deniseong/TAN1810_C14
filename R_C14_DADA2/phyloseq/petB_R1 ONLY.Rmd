---
title: "TAN1810 - petB R1 phyloseq"
author: "Denise Ong"
date: "9/6/2021"
output:
  prettydoc::html_pretty:
    ...
    math: katex
---
The data is from the 2x250 petB run, so only the R1 sequences were processed here. There are 4 phyloseq files here:
...
I compared for the sorted syn those that were assigned ESTU and non ESTU, but actually don't make sense to be assigned in two phyloseq because asv is assigned against the same sequences. So can just use the database with the ESTU and then plot to Subclade if not using ESTU. I do not think it changes the assignment! So 3 and 4 is assigned until ESTU level. 
...
ps <- read_rds("output_petb_2x250/output_sorted_syn_R1/phyloseq_asv_set_D1.1.RDS") 
...
p <- long %>%
  ...
  ggplot(aes(x=file_code, y=n_reads, color=Subclade, fill=Subclade)) + 
    ...
    #    facet_grid(.~water_mass, scales = "free_x") +
        ...
        ggtitle("C14 sorted syn R1") +
        scale_color_manual(values=subcladePalette, limits = force) + 
        scale_fill_manual(values=subcladePalette, limits = force)
p

pdf("phyloseq/figures/20210909_C14 subclade_subclade.pdf", height=5, width=8) ; plot(p) ; dev.off()
```
...
# Syn C14 - to ESTU level
```{r}
# Read data
# Load the phyloseq files
ps_ESTU <- read_rds("output_sorted_syn_unpaired/phyloseq_asv_set_D1.1_ESTU.RDS") 

#copy the column of interest to create new column, and replacing names for water masses SA and ST
sample_data(ps_ESTU)$water_mass <- sample_data(ps_ESTU)$Cycle.
sample_data(ps_ESTU)$water_mass = str_replace(sample_data(ps_ESTU)$water_mass, "1|2|5", "SA") 
sample_data(ps_ESTU)$water_mass = str_replace(sample_data(ps_ESTU)$water_mass, "3|4", "ST")

#copy the column of interest, creating new column and replace for cycle names. Not using this column for now. 
sample_data(ps_ESTU)$cycle_name <- sample_data(ps_ESTU)$Cycle.
sample_data(ps_ESTU)$cycle_name = str_replace(sample_data(ps_ESTU)$cycle_name, "1", "SA-Cycle_1")
sample_data(ps_ESTU)$cycle_name = str_replace(sample_data(ps_ESTU)$cycle_name, "2", "SA-Cycle_2") 
sample_data(ps_ESTU)$cycle_name = str_replace(sample_data(ps_ESTU)$cycle_name, "5", "SA-Cycle_5") 
sample_data(ps_ESTU)$cycle_name = str_replace(sample_data(ps_ESTU)$cycle_name, "3", "ST-Cycle_3")
sample_data(ps_ESTU)$cycle_name = str_replace(sample_data(ps_ESTU)$cycle_name, "4", "ST-Cycle_4")
...
# Do normalizations and transformation
ps_ESTU <- phyloseq_normalize_median(ps_ESTU)
long_ESTU <- phyloseq_transform_to_long(ps_ESTU)
```
...
p <- long_ESTU %>%
    ...
    ggplot(aes(x=vial, y=n_reads, color=Subclade, fill=Subclade)) + 
        geom_bar(position="fill", stat="identity") + 
        theme(text=element_text(size=10),axis.text.x = element_text(angle = 45, hjust = 1)) + 
        facet_grid(EXP.~cycle_name, scales = "free_x") +
        ggtitle("C14 sorted syn R1 ESTU level - Subclade") +
        scale_color_manual(values=subcladePalette, limits = force) + 
        scale_fill_manual(values=subcladePalette, limits = force)
p

#pdf("phyloseq/figures/20210909_C14 ESTU_subclade.pdf", height=5, width=8) ; plot(p) ; dev.off()
...
p <- long_ESTU %>%
  ...
  ggplot(aes(x=file_code, y=n_reads, color=ESTU, fill=ESTU)) + 
        ...
        theme(text=element_text(size=10),axis.text.x = element_text(angle = 45, hjust = 1)) + 
        facet_grid(.~cycle_name, scales = "free_x") +
        ggtitle("C14 sorted syn R1 ESTU level - ESTU") +
        scale_color_manual(values=ESTUPalette, limits = force) + 
        scale_fill_manual(values=ESTUPalette, limits = force)
p
pdf("phyloseq/figures/20210909_C14 ESTU_ESTU.pdf", height=5, width=8) ; plot(p) ; dev.off()
...
```{r}
# Load the phyloseq files
ps_ctd_nested <- read_rds("output_petb_2x250/output_CTD_petB_nested_R1/phyloseq_asv_set_D1.2.RDS")  # adding new component, this case the phyloseq object to the "tag" all
...
p <- long_ctd_nested %>%
  ...
  ggplot(aes(x=file_code, y=n_reads, color=Subclade, fill=Subclade)) + 
        ...
        theme(text=element_text(size=10),axis.text.x = element_text(angle = 45, hjust = 1)) + 
        facet_grid(.~cycle_location_name, scales = "free_x") +
        ggtitle("CTD petB nested R1 - subclade") +
        scale_color_manual(values=subcladePalette, limits = force) + 
        scale_fill_manual(values=subcladePalette, limits = force)
p

pdf("phyloseq/figures/20210916_CTD nested R1.pdf", height=5, width=8) ; plot(p) ; dev.off()
...
pdf("phyloseq/figures/20210909_CTD nested_ESTU.pdf", height=5, width=8) ; plot(p) ; dev.off()
...
```{r}
# Load the phyloseq files
ps_ctd_normal <- read_rds("output_petb_2x250/output_CTD_petB_normal_R1/phyloseq_asv_set_D1.3.RDS")  # adding new component, this case the phyloseq object to the "tag" all
...
pdf("phyloseq/figures/20210916_CTD normal_R1.pdf", height=5, width=8) ; plot(p) ; dev.off()
