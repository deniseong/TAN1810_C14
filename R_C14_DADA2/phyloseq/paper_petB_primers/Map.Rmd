...
pdf("phyloseq/figures_petb compare/map.pdf", height=6, width=5) ; plot(p) ; dev.off()
```
...
```{r}
library(ggOceanMaps)
basemap(data= expand.grid(lon = c(-176, 172), lat = c(-46, -40)), rotate = TRUE) +
    geom_spatial_point(data = sampling_location, aes(x = long, y = lat)) +
  scale_y_continuous(breaks = seq(-176, 172, by = 1)) +
  scale_x_continuous(breaks = seq(-46, -40, by = 1)) 
