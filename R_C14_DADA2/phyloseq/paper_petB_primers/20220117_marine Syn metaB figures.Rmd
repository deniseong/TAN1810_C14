---
title: "20220117_marine Syn metaB figures and tables"
author: "Denise Ong"
date: "1/17/2022"
...
library(here)
source(here("phyloseq", "init.R")) #for libraries
source(here("phyloseq", "init_markdown.R")) #for markdown libraries
source(here("phyloseq", "define_functions.R")) #functions to 1.transform to dataframe, 2.normalise and 3.create treemaps 
source(here("phyloseq", "colours_petb.R")) #colours
```

# Read data
```{r}
source(here("phyloseq", "read_petb.R"))
...
#export as PDF
pdf(here("phyloseq", "figures_petb compare", "read distribution_no rich_3.pdf"), height=8, width=7) ; plot(p) ; dev.off()
...
#proportion of reads per sample
#writexl::write_xlsx(join_filt_pct_wide, "subclade_percent_wide.xlsx")
```
...
g
pdf(here("phyloseq", "figures_petb compare","percent_distribution_asv.pdf"), height=10, width=7) ; plot(g) ; dev.off()
...
```{r}
pct_sim <- readxl::read_excel(here("phyloseq", "figures_petb compare","subclade_percent similarity.xlsx"), sheet = "Sheet2") 
...
#pdf(here("phyloseq", "figures_petb compare","pct_sim_refseq.pdf"), height=5, width=7) ; plot(p) ; dev.off()
```

# Figure 4. Taxonomic composition of sorted Synechococcus
```{r}
# edit variables in dataframe 
...
p <- long_sorted %>%
  ...
  ggplot(aes(x=n_reads, y=fct_rev(sample_name), color=Subclade, fill=Subclade)) + 
        ...
        ylab("Sample name")
p
#pdf(here("phyloseq", "figures_petb compare","read distribution_sorted syn.pdf"), height=5, width=7) ; plot(p) ; dev.off()
  
```

# Table S4. For each subclade, percentage of reads and wilcoxon test for sorted samples 
```{r}
...
# Figure S2. Taxonomic composition of filtered (Mazard_2012 and Ong_2022 protocol) and sorted samples
```{r}
long_sorted <- long_sorted %>%
...
pdf("phyloseq/figures_petb compare/tax_composition_all.pdf", height=9, width=7) ; plot(p) ; dev.off()
