...
# Singletons -How many reads per ASV in major clade
```{r}
...
# Barplot, filtered and sorted samples
```{r}
long <- long %>%
  filter(vial == "i") %>%
  dplyr::rename(depth = DEPTH,
         ctd_cast = `CTD.`)
long$depth <- as.character(long$depth)
long$station <- as.character(long$station)

join_all_ps <- full_join(join_ps, long) %>%
  replace_na(list(type = "Sorted sample")) %>%
  mutate(ctd_depth = paste(ctd_cast, depth, sep = '_'))

join_all_ps$cycle_station <- factor(join_all_ps$cycle_station, levels = c("ST1_1", "ST1_2", "ST1_3", "ST1_4", "ST2_1", "ST2_2","ST2_3", "SA1_1", "SA1_2", "SA1_3", "SA1_4", "SA1_5", "SA2_1", "SA2_2", "SA2_3", "SA2_4", "SA3_1", "SA3_2"))
