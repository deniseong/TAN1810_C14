...
pp_cal_1_join_edit <- pp_cal_1_join %>%
  ...
  rename(pp_filtered = pp)

pp_join <- left_join(pp_cal_1_join, pp_cal_1_join_edit)

pp_join$cycle <- factor(pp_join$cycle_name,levels = c("Subtropical 1", "Subtropical 2", "Subantarctic 1", "Subantarctic 2", "Subantarctic 3")) 
pp_join$population <- factor(pp_join$population,levels = c("Syn", "Pico", "Nano")) 

cycle_boxplot <- pp_join %>%
                 ggplot() +
                 geom_boxplot(aes(y=pp_filtered, x= population , fill = population, colour = population), show.legend = FALSE) + 
                 geom_point(aes(y=pp, x= population))+
                 facet_grid(.~cycle_name)+
                 scale_y_continuous(trans='log10') +
                 ylab("Primary Production rate\n(fgC/h/cell)") +
                 xlab("Cycle") +
                 scale_fill_manual(values = c("#FFCC00", "#56B4E9", "#009E73")) +
                 scale_colour_manual(values = c("#C4961A", "#0000CC", "#003333"))
