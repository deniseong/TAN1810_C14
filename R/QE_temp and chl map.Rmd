...
```{r}
#subset data for NZ area (coordinates lat -47 to -41, lon 172 to -176)
lon_sub <- which( nc_SST$dim$lon$vals > 172 | nc_SST$dim$lon$vals < -176)
lat_sub <- which( nc_SST$dim$lat$vals > -47 & nc_SST $dim$lat$vals < -41)
...
plot(raster_temp_nz)
```
Plot looks ok, now to convert to ggplot format with geom_raster?
