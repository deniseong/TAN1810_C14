...
# Read table for C14 cold values, merge for cycle and experiment number. Add new column to indicate dark.
```{r}
...
# NPP standard vs cold for each experiment
```{r}
...
df3 <- data_summary(cold_mean_pivot, varname="NPP", 
                    groupnames=c("type", "cycle_name"))
df3$cycle_name=as.factor(df3$cycle_name)
head(df3)

cold_mean_bar <- ggplot(data=df3, aes(x=cycle_name, y=NPP, fill=type))+
...
cold_mean_bar               
```
Overall difference betwen cold and standard. In this graph the difference does not look too big, but it is masked by combining the surface and DCM together. 
...
df4 <- data_summary_1(cold_mean_pivot, varname="NPP", 
...
cold_exp_bar <- ggplot(data=df4, aes(x=exp_name, y=NPP, fill=type))+
...
Comparing standard vs cold experiment, keeping either surface or DCM constant
```{r}

cold_surface <- cold_mean_pivot %>%
  filter(sample == "SUR")

surface_plot <- ggplot(data=cold_surface, aes(x=exp_name, y= NPP, fill = type))+
  geom_bar(stat="identity", position=position_dodge())+
  theme_bw()+
  ylab("Net Primary Production (mgC/m3/day)")+
  xlab("Experiment") +
...
cold_dcm <- cold_mean_pivot %>%
  filter(sample =="DCM")
DCM_plot <- ggplot(data=cold_dcm, aes(x=exp_name, y= NPP, fill = type))+
  geom_bar(stat="identity", position=position_dodge())+
  theme_bw()+
  ylab("Net Primary Production (mgC/m3/day)")+
  xlab("Experiment") +
...
surface vs DCM, keeping the type of experiment (standard/cold) constant.
```{r}
...
# To check the replicates of those with large differences.
```{r}
